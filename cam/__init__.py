...
import torchvision.transforms.functional as transFun
...
def apply_transforms(image, size=224):
    ...
    if not isinstance(image, Image.Image):
        image = transFun.to_pil_image(image)
...
def denormalize(tensor):
    ...
    # bug report: cloned tensor cannot be inplace modified . use detach
...
def basic_visualize(input_, gradients, save_path=None, weight=None, cmap='viridis', alpha=0.7):
    ...
    gradients = format_for_plotting((gradients))

    subplots = [
        ('Saliency map across RGB channels', [(gradients, cmap, None)]),
